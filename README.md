...
`VICE` can be used for any data type. We provide a file called `tripletize.py` that converts (latent) representations from any domain (e.g., fMRI, EEG, DNNs) corresponding to some set of stimuli (e.g., images, words) into an `N x 3` matrix of `N` triplets. We do this by exploiting the similarity structure of the representations.
